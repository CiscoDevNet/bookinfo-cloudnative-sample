...
/**
 * Set enable/disable flag for analytics agent.
 * @param enable
 *    if non zero, then analytics agent is enabled.
 *    if zero, then analytics agent is disabled (default behaviour)
 */
 ...
 * @return
 *     An handle for the custom event that is being defined. It will be NULL if failed to start the
 ...
 * the custom events in controller.
 *
 * @param appd_event_handle
 *     An handle for the custom event.
 *     If this is NULL, zero or invalid then its an error and zero is returned.
 * @param property_name
 *     A string containing the name of property.
 *     If this is NULL, zero or empty then its an error and zero is returned.
 * @param property_value
 *     A string containing the value of property.
 *     If this is NULL, zero or empty then its an error and zero is returned.
 * @return
 *     Returns non-zero if property was successfully added to custom event definition. Otherwise
 *     zero is returned.
 */
 ...
 *     If this is NULL, zero or empty then its an error and zero is returned.
 * @return
 *     Returns non-zero if detail was successfully added to custom event definition. Otherwise
 *     zero is returned.
 */
 ...
 * memory holding the even data will be kept alive till appd_sdk_term() is called. 
 *
 * @param appd_event_handle
 *     An handle for the custom event.
 *     If this is NULL, zero or invalid then its an error and zero is returned.
...
public:
   ...
   *     A string contaning the application context name for this custom event. If application
   *     context is empty then "default" agent is picked.
   * @param severity
   *     Enum representing the severity for this custom event.
   * @param eventSubType
   *     A string containing the custom event subtype. This subtype can be used in controller to
   ...
   *     A string containing the summary of what this custom event is about.
   * @param properties
   *     A map containing the property-name as key and property-value as value. These property-name
   *     and property-values CANNOT be empty, but an empty map can be passed in if there are no
   *     properties to pass.
   * @param details
   *     A map containing the detail-name as key and detail-value as value. These detail-name
   ...
   *
   * @return bool
   *    Returns true on successful reporting of custom event. Otherwise, reports false. 
   */
  bool report()
  {
    if (! m_success) {
